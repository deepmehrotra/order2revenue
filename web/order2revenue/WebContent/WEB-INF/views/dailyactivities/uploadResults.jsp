							...
							<c:when test="${mapType=='expensesMap'}">
                                  ...
                                  <c:set var="expense" value="${expensesMap.value}" />
	                                   ...
	                                   <td>${expense.expenseDescription}</td>
									    ...
									    <c:when test="${mapType=='orderReturnMap'}">
									  ...
									  <table class="table table-striped table-bordered table-hover dataTables-example" >
								       ...
								       <tr>
		                                ...
		                                <tbody>
		                                   <c:if test="${!empty orderReturnMap}">
                                  <c:forEach items="${orderReturnMap}" var="orderReturnMap" varStatus="loop">
                                  <c:set var="orderReturn" value="${orderReturnMap.value}" />
		                                <tr>
	                                    <td><input type="checkbox"></td>
	                                    <td>${loop.index+1}</td>
	                                    <td>${orderReturn.channelOrderID}</td>
	                                   <td>${orderReturn.subOrderID}</td>
	                                    <td>${orderReturn.productSkuCode}</td>
	                                    <td>${orderReturn.orderReturnOrRTO.returnOrRTOId}</td>
	                                    <td>${orderReturn.orderReturnOrRTO.returnOrRTOreason}</td>
	                                     <td>${orderReturn.orderReturnOrRTO.returnDate}</td>
	                                       <td>${orderReturn.orderReturnOrRTO.returnorrtoQty}</td>
	                                    <td>${orderReturn.orderReturnOrRTO.returnOrRTOstatus}</td>
	                                    <td>${orderReturn.orderReturnOrRTO.returnOrRTOChargestoBeDeducted}</td>
	                                      <td style="color:red;font-weight:bold">${orderReturnMap.key}</td>
	                                   </tr>
	                                	</c:forEach>
	                                	</c:if>
								 </tbody>
