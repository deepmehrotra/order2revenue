                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content overflow-h">
                            <form:form method="POST" action="saveProduct.html" id="addProductForm"
                            ...
                            <div class="col-sm-12">
                                <div class="col-sm-6">
                                 <div class="mar-btm-20-oh"><label class="col-sm-5 control-label">Title</label>
                                    ...
                                    <div><label class="col-sm-5 control-label">Price</label>
                                    <div class="col-sm-7"><form:input path="productPrice" value="${product.productPrice}"
                                     class="form-control"/></div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                
                                    <div class="mar-btm-20-oh"><label class="col-sm-4 control-label">Product Category </label>
                                    <div class="col-sm-8">
                                     <c:choose>
                                     <c:when test="${empty product.categoryName}">
                                    <form:select path="categoryName" items="${categoryMap}" class="form-control"
                                    id="categoryName">  
       								 </form:select>
       								 </c:when>
       								 <c:otherwise>
       								 <form:input path="categoryName" value="${product.categoryName}"
                                     class="form-control" readonly="true"/>
       								 </c:otherwise>
       								 </c:choose>
       								 </div>
                                    </div>
                                    <div class="mar-btm-20-oh"><label class="col-sm-4 control-label">Stock</label>
                                    <div class="col-sm-8">   <c:choose>
                                    	 <c:when test="${empty product.productSkuCode}">
                                    	 <form:input path="quantity"   id="quantity"   class="form-control"/></c:when>
                                     <c:otherwise> <form:input path="quantity" value="${product.quantity}"  
                                     class="form-control" readonly="true"/> </c:otherwise>
                                     </c:choose></div>
                                      </div>
                                    <div class="mar-btm-20-oh"><label class="col-sm-4 control-label">Threshold Limit</label>
                                    <div class="col-sm-8"><form:input path="threholdLimit" value="${product.threholdLimit}"
                                     class="form-control"/></div>
                                    </div>
                                    <div class="mar-btm-20-oh"><label class="col-sm-4 control-label">Channel SKU</label>
                                    <div class="col-sm-8"><form:input path="channelSKU" value="${product.channelSKU}"
                                     class="form-control"/></div>
                                    </div>
                                ...
                                <div class="hr-line-dashed"></div>
                                    <button class="btn btn-primary pull-right" type="button" onclick="submitProduct()">Save</button>
                                </div>
                            </form:form>
                        </div>
                </div>
