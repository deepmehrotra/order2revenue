...
<script type="text/javascript">
	...
	function onclickEditTaxCategory(id) {
		$.ajax({
			url : 'editTaxCategory.html?taxCategoryId=' + id,
			success : function(data) {
				if ($(data).find('#j_username').length > 0) {
					window.location.href = "orderindex.html";
				} else {
						...
						<div class="ibox float-e-margins">
							<div class="ibox-title">
								...
								<div class="ibox-tools">
									<a href="#" onclick="onclickAddTaxCategory()"
										class="btn btn-primary btn-xs">Add Tax Category</a>
								</div>
							</div>
								...
								<table class="table table-bordered custom-table" id="taxCatTable">
									<thead>
										<tr>
											...
											<th>Percentage</th>
											<th>Type</th>
											<th>Mapped Product Category</th>
											<th>Action</th>
										</tr>
									</thead>
									<tbody>
										<c:if test="${!empty taxCategories}">
											<c:forEach items="${taxCategories}" var="category"
												varStatus="loop">
												<tr>
													<td>${loop.index+1}</td>
													...
													<td><div style="overflow-y: scroll;max-height: 300px;">${category.productCategoryStr}</div></td>
