...
public ModelAndView searchReturnOrder(HttpServletRequest request,@ModelAttribute("command")OrderBean orderBean,
   BindingResult result) {
	  ...
	  if(searchType.equals("searchByProperty")&&searchColumn!=null)
	  {
		  ...
		  else
		  {
			  System.out.println(" Inside search find orders");
			   orderlist=ConverterClass.prepareListofBean(orderService.findOrders(searchColumn, searchString, HelperClass.getSellerIdfromSession(request)));
		  }
	  }
	  else
	  {
		 orderlist=new ArrayList<OrderBean>();
		  ...
		  {
			  temporaryorderlist=orderService.findOrdersbyPaymentDate(searchDateCriteria, startDate, endDate, HelperClass.getSellerIdfromSession(request));
...
public ModelAndView saveReturnorRTO(HttpServletRequest request,@ModelAttribute("command")OrderBean orderBean,
   BindingResult result) {
	 Map<String, Object> model = new HashMap<String, Object>();
	 List<OrderBean> orderlist=null;
	 System.out.println("Inside savereturn order : channelordrid : "+orderBean.getChannelOrderID());
	 System.out.println(" orderid : "+orderBean.getOrderId()+" return id : "+orderBean.getOrderReturnOrRTO().getReturnOrRTOId()
			 +" return charge : "+orderBean.getOrderReturnOrRTO().getReturnOrRTOChargestoBeDeducted()
			 +"return date : "+orderBean.getOrderReturnOrRTO().getReturnDate());


	 if(orderBean.getChannelOrderID()!=null&&orderBean.getOrderReturnOrRTO().getReturnOrRTOId()!=null)
	 {
		  orderService.addReturnOrder(orderBean.getChannelOrderID(),ConverterClass.prepareOrderRTOorReturnModel(orderBean.getOrderReturnOrRTO()),HelperClass.getSellerIdfromSession(request));

	 }

	  System.out.println(" Returnorders list : "+orderlist);
	//  model.put("orders",  ConverterClass.prepareListofBean(orderService.listOrders(HelperClass.getSellerIdfromSession(request),0)));
...
System.out.println("*** Returnid  : "+request.getParameter("returnId"));
  ...
  {
  if(searchType!=null&&searchType.equals("searchByProperty")&&searchColumn!=null)
  {
	  if(searchColumn.equals("customerName")||searchColumn.equals("customerCity")||
			  searchColumn.equals("customerEmail")||searchColumn.equals("customerPhnNo"))
	  {
