		...
		List<CategoryBean> categorylist=null;
 		Map<String, Object> model = new HashMap<String, Object>();
 		 ...
 		 if(catId!=null)
 		 {
 			 try
 			 {
 		categorylist=ConverterClass.prepareListofCategoryBean(categoryService.listParentCategories(HelperClass.getSellerIdfromSession(request)));
...
public ModelAndView saveCatInventory(@ModelAttribute("command")CategoryBean categoryBean,
   BindingResult result,HttpServletRequest request) {
	 ...
	 log.debug(" ********* parent category name :"+categoryBean.getParentCatName());
	 Map<String, Object> model = new HashMap<String, Object>();
...
public @ResponseBody String checkInventoryGroup(HttpServletRequest request,@ModelAttribute("command")CategoryBean categoryBean,
  BindingResult result) {
	...
	{
		Category cat=categoryService.getCategory(name, HelperClass.getSellerIdfromSession(request));
		if(cat!=null)
			return "false";
		else
			return "true";
...
public @ResponseBody String checkTaxCategory(HttpServletRequest request,@ModelAttribute("command")FormBean formBean,
		   BindingResult result) {
	 log.info("***checkTaxCategory Start****");
	 log.debug("tax category name : "+request.getParameter("name"));
	 try
	 {
		TaxCategory taxCategory=	taxDetailService.getTaxCategory(request.getParameter("name"), HelperClass.getSellerIdfromSession(request));
		if(taxCategory!=null){
		return  "false";
	}else{
		return  "true";
