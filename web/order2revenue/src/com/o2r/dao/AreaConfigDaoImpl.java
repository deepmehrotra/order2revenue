...
@Override
	public List<String> listCountryStates(String country) {
		...
		try {
			...
			Query gettingStates = session
					.createSQLQuery(stateRetriveQuery)
	...
	public String getStateFromZipCode(String zipcode) {
		...
		try {
			...
			session.beginTransaction();
			Query gettingStates = session
					.createSQLQuery(stateRetriveFromZipCodeQuery)
	 ...
	 public String getCityFromZipCode(String zipcode)
	 {
			...
			try {
				...
				session.beginTransaction();
				Query gettingStates = session
						.createSQLQuery(cityRetriveFromZipCodeQuery)
						.setParameter("zipcode", zipcode);
	 ...
	 public String getStateFromCity(String city)
	 {
			...
			try {
				...
				session.beginTransaction();
				Query gettingStates = session
						.createSQLQuery(stateRetriveFromCityQuery)
